...
/**
 * Funding chart uses IndicatorEngine (async via virtual thread).
 */
public class FundingChart extends IndicatorChart {

    private Runnable requestRedraw;

    public FundingChart() { super(IndicatorType.FUNDING); }

    public void setRequestRedraw(Runnable requestRedraw) { this.requestRedraw = requestRedraw; }
    ...
    protected boolean render(XYPlot plot, List<Candle> candles, ChartDataContext ctx) {
        // Rendering is handled in subscribe() via async callback
        return false;
    }
