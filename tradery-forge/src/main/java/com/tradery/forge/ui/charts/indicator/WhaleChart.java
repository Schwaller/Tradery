...
public class WhaleChart extends IndicatorChart {
    ...
    public void subscribe(ChartDataContext ctx) {
        ctx.indicatorDataService().subscribeWhaleDelta(threshold);
    }

    @Override
    protected boolean render(XYPlot plot, List<Candle> candles, ChartDataContext ctx) {
        double[] whaleDelta = ctx.indicatorDataService().getWhaleDelta(threshold);
