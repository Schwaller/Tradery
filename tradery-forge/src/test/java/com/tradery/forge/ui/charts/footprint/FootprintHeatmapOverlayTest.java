...
import com.tradery.dataclient.DataServiceClient.RawProfileResponse;
...
class FootprintHeatmapOverlayTest {
    ...
    @Nested
    @DisplayName("requiresAggTrades")
    class RequiresAggTradesTest {

        @Test
        void combinedDoesNotRequireAggTrades() {
            assertFalse(FootprintHeatmapOverlay.requiresAggTrades(FootprintDisplayMode.COMBINED));
        }

        @Test
        void splitDoesNotRequireAggTrades() {
            assertFalse(FootprintHeatmapOverlay.requiresAggTrades(FootprintDisplayMode.SPLIT));
        }

        @Test
        void singleExchangeRequiresAggTrades() {
            assertTrue(FootprintHeatmapOverlay.requiresAggTrades(FootprintDisplayMode.SINGLE_EXCHANGE));
        }

        @Test
        void stackedRequiresAggTrades() {
            assertTrue(FootprintHeatmapOverlay.requiresAggTrades(FootprintDisplayMode.STACKED));
        }

        @Test
        void divergenceRequiresAggTrades() {
            assertTrue(FootprintHeatmapOverlay.requiresAggTrades(FootprintDisplayMode.DIVERGENCE));
        }
    }
    ...
    class BuildFootprintFromProfileTest {
        ...
        /**
         * Invoke the private static buildFootprintFromProfile via reflection.
         */
        private Footprint invokeBuilder(RawProfileResponse profile, double tickSize, Candle candle) throws Exception {
            Method method = FootprintHeatmapOverlay.class.getDeclaredMethod(
                "buildFootprintFromProfile", Footprint.Builder.class,
                RawProfileResponse.class, double.class, Candle.class);
            ...
            method.invoke(null, fpBuilder, profile, tickSize, candle);
            return fpBuilder.build();
        }

        @Test
        @DisplayName("null profile produces empty buckets covering candle range")
        void nullProfileProducesEmptyBuckets() throws Exception {
        ...
        void separateProfileTicksStaySeparate() throws Exception {
            ...
            RawProfileResponse profile = new RawProfileResponse(1000L, 1.0, 90.0, 50.0, levels);
        ...
        void profileTicksAccumulateAtSameLevel() throws Exception {
            ...
            RawProfileResponse profile = new RawProfileResponse(1000L, 1.0, 60.0, 45.0, levels);
        ...
        void accumulationPreservesTotalVolume() throws Exception {
            ...
            RawProfileResponse profile = new RawProfileResponse(1000L, 0.1, expectedTotalBuy, expectedTotalSell, levels);
        ...
        void emptyLevelsProducesEmptyBuckets() throws Exception {
            RawProfileResponse profile = new RawProfileResponse(1000L, 1.0, 0, 0, Map.of());
    ...
    class CalculateATRTest {
        ...
        private double invokeATR(List<Candle> candles, int period) throws Exception {
            Method method = FootprintHeatmapOverlay.class.getDeclaredMethod(
                "calculateATR", List.class, int.class);
            method.setAccessible(true);
            return (double) method.invoke(null, candles, period);
        }

        @Test
        void singleCandleReturnsHighMinusLow() throws Exception {
            Candle c = new Candle(1000L, 100.0, 110.0, 90.0, 105.0, 1000.0,
                100, 500.0, 300.0, 150.0);
            double atr = invokeATR(List.of(c), 14);
            assertEquals(20.0, atr, 0.001);
        }

        @Test
        void twoCandlesComputesTrueRange() throws Exception {
            Candle c1 = new Candle(1000L, 100.0, 110.0, 90.0, 105.0, 1000.0,
                100, 500.0, 300.0, 150.0);
            Candle c2 = new Candle(2000L, 106.0, 115.0, 95.0, 112.0, 1000.0,
                100, 500.0, 300.0, 150.0);
            double atr = invokeATR(List.of(c1, c2), 14);
