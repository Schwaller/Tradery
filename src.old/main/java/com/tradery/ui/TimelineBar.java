...
public class TimelineBar extends JPanel implements DataPageListener<Candle> {
    ...
    private boolean isOverWindow(int x) {
        ...
        // Ensure minimum drag zone width of 16px for easier interaction
        double minDragWidth = 16.0;
        double currentWidth = x2 - x1;
        if (currentWidth < minDragWidth) {
            double expand = (minDragWidth - currentWidth) / 2.0;
            x1 -= expand;
            x2 += expand;
        }
