 ...
 * Subscribes to RetailDeltaCompute in the constructor; the onReady callback
 * handles both initial rendering and recomputation.
 */
public class RetailRenderer implements IndicatorChartRenderer {

    private static final Color RETAIL_BUY_COLOR = new Color(129, 199, 132);
    private static final Color RETAIL_SELL_COLOR = new Color(239, 154, 154);

    private final double threshold;
    private final IndicatorSubscription<double[]> subscription;

    public RetailRenderer(double threshold, XYPlot plot, ChartDataProvider provider) {
        this.threshold = threshold;

        IndicatorPool pool = provider.getIndicatorPool();
        this.subscription = pool.subscribe(new RetailDeltaCompute(threshold));
        subscription.onReady(retailDelta -> {
            if (retailDelta == null || retailDelta.length == 0) return;

            clearPlot(plot);
            ChartStyles.addChartTitleAnnotation(plot, "Retail Delta");
