...
public class VolumeRatioChart extends IndicatorChartBase {
    ...
    public void subscribe(ChartDataContext ctx) {
        ctx.indicatorDataProvider().subscribeBuyVolume();
        ctx.indicatorDataProvider().subscribeSellVolume();
    }

    @Override
    protected boolean render(XYPlot plot, List<Candle> candles, ChartDataContext ctx) {
        double[] buyVolume = ctx.indicatorDataProvider().getBuyVolume();
        double[] sellVolume = ctx.indicatorDataProvider().getSellVolume();
