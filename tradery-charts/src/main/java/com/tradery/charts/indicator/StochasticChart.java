...
public class StochasticChart extends IndicatorChartBase {
    ...
    public void subscribe(ChartDataContext ctx) {
        ctx.indicatorDataProvider().subscribeStochastic(kPeriod, dPeriod);
    }

    @Override
    protected boolean render(XYPlot plot, List<Candle> candles, ChartDataContext ctx) {
        var result = ctx.indicatorDataProvider().getStochastic(kPeriod, dPeriod);
