...
/**
 * Premium Index chart. Uses constructor-injected PremiumDataSource for async data loading,
 * with IndicatorEngine fallback.
 */
public class PremiumChart extends IndicatorChartBase {

    /**
     * Interface for async premium data loading.
     * Forge implements this via PremiumPageManager; Desk passes null (not supported).
     */
    public interface PremiumDataSource {
        DataPageView<PremiumIndex> request(String symbol, String timeframe, long start, long end,
                                            DataPageListener<PremiumIndex> listener, String consumer);
        void release(DataPageView<PremiumIndex> page, DataPageListener<PremiumIndex> listener);
    }

    private final PremiumDataSource premiumDataSource; // nullable
    ...
    public PremiumChart(PremiumDataSource premiumDataSource) {
        super(IndicatorType.PREMIUM);
        this.premiumDataSource = premiumDataSource;
    }

    @Override
    public void subscribe(ChartDataContext ctx) {
        if (premiumDataSource != null) {
            subscribePremiumPage(ctx);
        } else if (ctx.indicatorEngine() != null) {
            subscribeViaEngine(ctx);
        }
    }

    private void subscribePremiumPage(ChartDataContext ctx) {
        if (premiumPage != null && premiumListener != null) {
            premiumDataSource.release(premiumPage, premiumListener);
        }
        ...
        premiumPage = premiumDataSource.request(ctx.symbol(), ctx.timeframe(), ctx.startTime(), ctx.endTime(), premiumListener, "PremiumChart");
    }
    ...
    @Override
    public void dispose() {
        if (premiumDataSource != null && premiumPage != null && premiumListener != null) {
            premiumDataSource.release(premiumPage, premiumListener);
        }
        premiumPage = null;
