...
public class RetailChart extends IndicatorChartBase {
    ...
    public void subscribe(ChartDataContext ctx) {
        ctx.indicatorDataProvider().subscribeRetailDelta(threshold);
    }

    @Override
    protected boolean render(XYPlot plot, List<Candle> candles, ChartDataContext ctx) {
        double[] retailDelta = ctx.indicatorDataProvider().getRetailDelta(threshold);
