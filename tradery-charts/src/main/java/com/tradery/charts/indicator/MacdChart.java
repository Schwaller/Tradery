...
public class MacdChart extends IndicatorChartBase {
    ...
    public void subscribe(ChartDataContext ctx) {
        ctx.indicatorDataProvider().subscribeMACD(fast, slow, signal);
    }

    @Override
    protected boolean render(XYPlot plot, List<Candle> candles, ChartDataContext ctx) {
        var macdResult = ctx.indicatorDataProvider().getMACD(fast, slow, signal);
