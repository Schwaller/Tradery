 ...
 * Each chart type is encapsulated in an {@link IndicatorChartBase} subclass.
 * Uses constructor injection for all external dependencies.
 */
public class IndicatorChartsManager {

    private final EnumMap<IndicatorType, IndicatorChartBase> charts = new EnumMap<>(IndicatorType.class);
    private final EnumSet<IndicatorType> enabledIndicators = EnumSet.noneOf(IndicatorType.class);

    private final IndicatorDataProvider dataProvider;
    ...
    public IndicatorChartsManager(IndicatorDataProvider dataProvider,
                                   PremiumChart.PremiumDataSource premiumDataSource,
                                   SpectrumChart.SpectrumDataSource spectrumDataSource,
                                   SpectrumChart.SpectrumConfig spectrumConfig) {
        this.dataProvider = dataProvider;
        registerCharts(premiumDataSource, spectrumDataSource, spectrumConfig);
        dataProvider.addDataListener(this::onIndicatorDataReady);
    }

    private void registerCharts(PremiumChart.PremiumDataSource premiumDataSource,
                                 SpectrumChart.SpectrumDataSource spectrumDataSource,
                                 SpectrumChart.SpectrumConfig spectrumConfig) {
        ...
        charts.put(IndicatorType.PREMIUM, new PremiumChart(premiumDataSource));
    ...
    public void setDataContext(List<Candle> candles, String symbol, String timeframe,
        ...
        dataProvider.setDataContext(candles, symbol, timeframe, startTime, endTime);

        ChartDataContext ctx = new ChartDataContext(
            symbol, timeframe, startTime, endTime,
            dataProvider, indicatorEngine, currentTrades
        );
    ...
    /**
     * Called when indicator data becomes ready (callback from IndicatorDataProvider).
     */
    ...
    public void updateCharts(List<Candle> candles) {
        ...
        for (IndicatorType type : enabledIndicators) {
            IndicatorChartBase chart = charts.get(type);
            chart.subscribe(ctx);
    ...
    public void refreshSpectrumChart() {
        IndicatorChartBase spectrum = charts.get(IndicatorType.SPECTRUM);
    ...
    /**
     * Apply all saved indicator chart settings (enable states + parameters) from ChartConfig.
     */
    public void applyConfig(ChartConfig config) {
        // Set parameters before enabling (so subscribe uses correct params)
        RsiChart rsi = getChartImpl(IndicatorType.RSI);
        rsi.setPeriod(config.getRsiPeriod());
        setEnabled(IndicatorType.RSI, config.isRsiEnabled());

        MacdChart macd = getChartImpl(IndicatorType.MACD);
        macd.setFast(config.getMacdFast());
        macd.setSlow(config.getMacdSlow());
        macd.setSignal(config.getMacdSignal());
        setEnabled(IndicatorType.MACD, config.isMacdEnabled());

        AtrChart atr = getChartImpl(IndicatorType.ATR);
        atr.setPeriod(config.getAtrPeriod());
        setEnabled(IndicatorType.ATR, config.isAtrEnabled());

        StochasticChart stoch = getChartImpl(IndicatorType.STOCHASTIC);
        stoch.setKPeriod(config.getStochasticKPeriod());
        stoch.setDPeriod(config.getStochasticDPeriod());
        setEnabled(IndicatorType.STOCHASTIC, config.isStochasticEnabled());

        RangePositionChart rp = getChartImpl(IndicatorType.RANGE_POSITION);
        rp.setPeriod(config.getRangePositionPeriod());
        setEnabled(IndicatorType.RANGE_POSITION, config.isRangePositionEnabled());

        AdxChart adx = getChartImpl(IndicatorType.ADX);
        adx.setPeriod(config.getAdxPeriod());
        setEnabled(IndicatorType.ADX, config.isAdxEnabled());

        setEnabled(IndicatorType.DELTA, config.isDeltaEnabled());
        setEnabled(IndicatorType.CVD, config.isCvdEnabled());
        setEnabled(IndicatorType.VOLUME_RATIO, config.isVolumeRatioEnabled());

        WhaleChart whale = getChartImpl(IndicatorType.WHALE);
        whale.setThreshold(config.getWhaleThreshold());
        setEnabled(IndicatorType.WHALE, config.isWhaleEnabled());

        RetailChart retail = getChartImpl(IndicatorType.RETAIL);
        retail.setThreshold(config.getRetailThreshold());
        setEnabled(IndicatorType.RETAIL, config.isRetailEnabled());

        setEnabled(IndicatorType.TRADE_COUNT, config.isTradeCountEnabled());
        setEnabled(IndicatorType.FUNDING, config.isFundingEnabled());
        setEnabled(IndicatorType.OI, config.isOiEnabled());
        setEnabled(IndicatorType.PREMIUM, config.isPremiumEnabled());
        setEnabled(IndicatorType.FEAR_GREED, config.isFearGreedEnabled());
    ...
    public <T extends IndicatorChartBase> T getChartImpl(IndicatorType type) {
    ...
    public IndicatorDataProvider getDataProvider() {
        return dataProvider;
    }
    ...
    public void addMouseWheelListener(java.awt.event.MouseWheelListener listener) {
        for (IndicatorChartBase chart : charts.values()) {
