...
public class TradeCountChart extends IndicatorChartBase {
    ...
    public void subscribe(ChartDataContext ctx) {
        ctx.indicatorDataProvider().subscribeTradeCount();
    }

    @Override
    protected boolean render(XYPlot plot, List<Candle> candles, ChartDataContext ctx) {
        double[] tradeCount = ctx.indicatorDataProvider().getTradeCount();
